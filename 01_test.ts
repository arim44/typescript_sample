...
function sayHi(name:string):void { //반환값 없음
    console.log(`Hi ${name}`);
}
...
function greeting1(name: string, callback:(name:string) => string) : string{
    return callback(name);
}

const result1 = greeting1('손흥민', sayHello2);
const result2 = greeting1('이강인', (name:string)=> `Hello ${name}`);
...
// 매개변수 타입과 반환형
function getDan(dan:number):number{
    return dan;
}

// 출력
function showMultiple(dan:number){
    let result;
    console.log(`====== ${dan}단 ======`);
    for(let i=1; i<10; i++){
        result = `${dan} * ${i} = ${dan * i}`;
